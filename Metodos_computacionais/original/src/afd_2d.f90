...
do ix=1,nxx
   do iz=1,nzz
      vel(iz,ix)=(vel(iz,ix)*dt/dx)**2/2
!      velz(iz,ix)=(vel(iz,ix)*dt/dz)**2/2	
   end do
end do
do ix=1,nxx
   do iz=1,nzz
      velz(iz,ix)=(velz(iz,ix)*dt/dx)**2/2
!      velz(iz,ix)=(vel(iz,ix)*dt/dz)**2/2	
   end do
end do
!velz(iz,ix)=(vel(iz,ix)*dt/dz)**2/2
...
!                              )
      p(iz,ix,1) = invpgama * (                                                         & 
                                2.0_dp*p(iz,ix,2)-mgama*p(iz,ix,1)   +                  & 
                                vel(iz,ix)*( laplacian + fonte * sourcemask(iz,ix) )  + &
                                velz(iz,ix)*( laplacian + fonte * sourcemask(iz,ix) )   &
                              )
    end do
...
close(SUMVID)
!
deallocate(p,vel,gama_x,gama_z,velz)
!                     
end program afd_2d
