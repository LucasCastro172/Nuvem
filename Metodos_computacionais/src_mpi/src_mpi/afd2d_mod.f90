...
real(dp),pointer:: pfield1(:,:)=>null()       ! campo de onda
real(dp),pointer:: pfield2(:,:)=>null()       ! campo de onda
real(dp),pointer:: p1(:,:)=>null()        ! campo de onda
real(dp),pointer:: p2(:,:)=>null()        ! campo de onda
real(dp),pointer:: fswap(:,:)=>null()     ! auxiliar pointer
real(sp),allocatable:: illum(:,:)
real(sp),allocatable:: rtm(:,:)           ! modelo de velocidade
!
...
if ( associated(this%p1) ) this%p1=>null()
if ( associated(this%p2) ) this%p2=>null()
if ( associated(this%fswap) ) this%fswap=>null()
if ( associated(this%pfield1) ) then
     deallocate(this%pfield1)
     this%pfield1=>null()
end if
if ( associated(this%pfield2) ) then
     deallocate(this%pfield2)
     this%pfield2=>null()
end if

if ( allocated(this%gamma1) ) then
     deallocate(this%gamma1)
end if

if ( allocated(this%gamma2) ) then
     deallocate(this%gamma2)
end if

if ( allocated(this%gradwind1) ) then
     deallocate(this%gradwind1)
end if

if ( allocated(this%gradwind2) ) then
     deallocate(this%gradwind2)
end if

if ( allocated(this%wsrc1) ) then
     deallocate(this%wsrc1)
end if

if ( allocated(this%wsrc2) ) then
     deallocate(this%wsrc2)
end if

if ( allocated(this%i1src) ) then
     deallocate(this%i1src)
end if

if ( allocated(this%i2src) ) then
     deallocate(this%i2src)
end if
if ( allocated(this%wrcv1) ) then
     deallocate(this%wrcv1)
end if

if ( allocated(this%wrcv2) ) then
     deallocate(this%wrcv2)
end if

if ( allocated(this%i1rcv) ) then
     deallocate(this%i1rcv)
end if

if ( allocated(this%i2rcv) ) then
     deallocate(this%i2rcv)
end if

if ( allocated(this%source_pulse) ) then
     deallocate(this%source_pulse)
end if
...
if ( allocated(this%wdata) ) then
     deallocate(this%wdata)
end if

if ( allocated(this%shotid) ) then
     deallocate(this%shotid)
end if


if ( allocated(this%rtm) ) then
     deallocate(this%rtm)
end if

if ( allocated(this%shotgather)  ) then
do itrc=1,this%ngmax
   if ( allocated(this%shotgather(itrc)%trcdata)) then
   deallocate(this%shotgather(itrc)%trcdata)
   end if
end do
deallocate(this%shotgather)
end if

if ( allocated(this%survey)  ) then
do itrc=1,this%ngmax
   if ( allocated(this%survey(itrc)%trcdata)) then
   deallocate(this%survey(itrc)%trcdata)
   end if
end do
deallocate(this%survey)
end if

if ( allocated(this%csgather)  ) then
     deallocate(this%csgather)
end if
...
subroutine parse_comments(fid)
  integer, intent(in) :: fid
  character(len=1) :: firstChar

  firstChar = '#'
  do while (firstChar .eq. '#')
    read(fid, '(A)') firstChar
  enddo
  backspace(fid)

end subroutine parse_comments

subroutine afd_rtm(this,datafile,iexit)
class(afd2d), intent(inout) :: this
character(*), intent(inout) :: datafile
integer, intent(out) :: iexit
!
! local
integer  :: ishot,ig,itrc,ns,it,itrec,frameindex
integer  :: i1,i2,i1m,i1p,i2m,i2p,ic,isrc,fid
real(dp) :: scalco,scalel,s1,s2,shotspread,datnorm
real(dp) :: t
real(sp) :: dnorm,laplacian,damp
integer :: ishot_beg
integer :: ishot_end
!
real(sp), allocatable :: rtm(:,:),ps(:,:),illum(:,:)
!
character(len=1024)::mvfile
integer :: nframe,recsize,ntrtm,ndtrtm
!
allocate(rtm(this%n1,this%n2),ps(this%n1,this%n2),stat=iexit)

if ( this%mpimyrank == this%mpimaster ) then
     allocate(this%rtm(this%n1,this%n2),stat=iexit)
     this%rtm (:,:) = 0.0
end if
!
rtm(:,:) = 0.0 ! guardar imagem 
illum(:,:) = 0.0 ! guardar imagem
ndtrtm=2*this%ndtrec !salvar a cada 8ms
...
this%p2(:,:) = 0.d0
!
! abrir arquivo para salvar frame do campo forward
!
mvfile(1:6)='movie_'
write(mvfile(7:11),'(i5.5)') this%mpimyrank
mvfile(12:15)='.bin'
!
fid=MVFID+this%mpimyrank
!
recsize=this%n1*this%n2*SP
open(FID,file=trim(mvfile),  &
           form='unformatted', &
           access='stream',    &
           action='write',     &
           status='replace',   &
...
call source_injection(this,0,t)
!
if ( mod(it-1,2*this%ndtrec) == 0.0 ) then
     !
     ntrtm=it
     nframe=nframe+1
     do i2=1,this%n2
     do i1=1,this%n1
        write(fid) real(this%p2(nborda+i1,nborda+i2),sp)
     end do
     end do
     !
end if
...
print *,'FORWARD MODELING DONE: ',this%mpimyrank,nframe
...
itrec=0
loop_tbackward: do it=ntrtm,1,-1                 ! evolucao do campo de pressao
!
t=real(it-1,dp)*this%dt
!
call time_update(this)
!
call source_injection(this,1,t)
!
if ( mod(it-1,2*this%ndtrec) == 0 ) then
     !
     read(fid,rec=nframe) ps
     nframe=nframe-1
     !
     do i2=1,this%n2
     do i1=1,this%n1
        rtm(i1,i2) = rtm(i1,i2) + ps(i1,i2)*real(this%p2(nborda+i1,nborda+i2),sp)
     end do
     end do
     !
end if
!
end do loop_tbackward
close(fid,status="DELETE")

end do loop_shots


call MPI_reduce(rtm,this%rtm,this%n1*this%n2, &
                MPI_REAL4,MPI_SUM,this%mpimaster,MPI_COMM_WORLD,iexit);

if ( this%mpimyrank == this%mpimaster ) then
     damp = 0.0
     do i2=1,this%n2
     do i1=1,this%n1
        damp=max(damp,this%illum(i1,i2))
     end do
     end do 
     damp = 1.e-5*damp
     ...
     do i2=1,this%n2
     do i1=1,this%n1
        laplacian= (this%scal1+this%scal2)*deriv2(1)*rtm(i1,i2)
        do ic=0,DRVLEN-1
           i1m=max(i1-ic,1)
           i1p=min(i1+ic,this%n1)
           i2m=max(i2-ic,1)
           i2p=min(i2+ic,this%n2)
           laplacian =  laplacian +  & 
                   deriv2(ic+1)*(this%scal1*(rtm(i1m,i2)+rtm(i1p,i2)) + &
                                 this%scal2*(rtm(i1,i2m)+rtm(i1,i2p)) ) 
        end do
        this%rtm(i1,i2)=-laplacian*this%vel(i1,i2) 
     end do
     end do
end if
call MPI_Barrier(MPI_COMM_WORLD,iexit)
...
subroutine afd_rtm_main()
!
type(afd2d)::afd
!
integer,parameter :: NAMELEN=1024
integer,parameter :: PARFID=72
integer,parameter :: IMGFID=23
!
integer::ns,nr,nt
!
character(NAMELEN) :: velhdr,sudata,suimage
!
real(dp) :: freq,wtime
!
integer :: i2,iexit,stat
!
integer :: shotbeg,shotend,shotstep
!
character(NAMELEN) :: parfile
!
character(1):: comment
...
!
!write(*,'("Entre vel. model header file: ")',advance='no')
call parse_comments(PARFID)
read(PARFID,'(A)') velhdr
!
!write(*,'("Entre freq. pulso fonte: ")',advance="no")
call parse_comments(PARFID)
read(PARFID,*) freq
!write(*,'("Entre SU input data file: ")',advance='no')
call parse_comments(PARFID)
read(PARFID,'(A)') sudata
!
!write(*,'("Entre SU output image file: ")',advance='no')
call parse_comments(PARFID)
read(PARFID,'(A)') suimage
!
call parse_comments(PARFID)
read(PARFID,*) shotbeg,shotend,shotstep
!
close(PARFID)
!
if ( afd%mpimyrank == afd%mpimaster ) then
   ...
   write(*,'("Data file: ",a)') trim(sudata)
   write(*,'("Freq. pulso fonte: ",f7.2," Hz")') freq
   write(*,'("Shotbeg, shotend, step : ",i5,i5,i5)')  shotbeg,shotend,shotstep
end if
!
wtime=MPI_WTIME()
!
...
call set_fd_solver(afd,freq,iexit)
!
call set_shots(afd,shotbeg,shotend,shotstep)
!
call afd_rtm(afd,sudata,iexit)
!
wtime=MPI_WTIME()-wtime
!
if ( afd%mpimyrank == afd%mpimaster ) then
!
     vtrace%trcheader%ns=afd%n1
     vtrace%trcheader%ntr=afd%n2
     vtrace%trcheader%f1=afd%o1
     vtrace%trcheader%f2=afd%o2
     vtrace%trcheader%d1=afd%d1
     vtrace%trcheader%d2=afd%d2
!
     allocate(vtrace%trcdata(afd%n1))
!
     iexit =  OpenSU(IMGFID,trim(suimage),'w',afd%n1)
!
     do i2=1,afd%n2
        vtrace%trcheader%fldr    = i2
        vtrace%trcdata(1:afd%n1) = afd%rtm(1:afd%n1,i2)
        iexit=PutSUTrace(IMGFID,i2,vtrace)
     end do
     close(IMGFID)
     deallocate(vtrace%trcdata)
     print *,'MPI_WTIME : ',wtime,mpi_wtick()
end if
call cleanup(afd)
!
