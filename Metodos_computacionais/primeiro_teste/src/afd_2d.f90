...
read(*,'(A)') velhdr
write(*,'("Entre com a posicao da fonte (zs,xs) : ")',advance='no')
read(*,*) zf,xf
write(*,'("Entre com a posicao do geofone (zs,xs) : ")',advance='no')
read(*,*) zr,xr
write(*,'("Entre freq. pico do pulso fonte: ")',advance='no')
read(*,*) freq
write(*,'("Entre tempo total de modelagem: ")',advance='no')
read(*,*) ttotal
write(*,'("Entre arquivo de saida (.su): ")',advance='no')
read(*,*) sumovie
!
...
if (ierr/=0) then                      ! informa falha na alocacao de memoria
    stop 'falha de alocaรงao da malha'
end if

allocate(vtrace%trcdata(nz),stat = ierr)
if (ierr/=0) then                      
        stop 'falha de alocaรงao do traco'
end if
!
...
!!!!!!!!!!!!FICAR DE OLHO PARA A PARTE COM ESPAÇAMENTOS
...
! Pulso Ricker:
        ...
        fonte = (dz*dx)*exp(-(delt))*(1.0_dp-2.0_dp*delt) ! pulso Ricker
!        
!FD scheme:
...
do ix=DRVLEN,nxx-DRVLEN
   do iz=2,DRVLEN
      ...
      p(iz,ix,1) = invpgama * (                                                      & 
                                ...
                                vel(iz,ix)*(rdz)*( laplacian + fonte * sourcemask(iz,ix) ) &
                              )
    end do
...
loop_t :do it=1,nt                    ! Loop de evolucao do campo de pressao sem superficie livre
!
! Pulso Ricker:
        delt  = (beta*real(it-it0,dp))**2
        fonte = (dz*dx)*exp(-(delt))*(1.0_dp-2.0_dp*delt) ! pulso Ricker
