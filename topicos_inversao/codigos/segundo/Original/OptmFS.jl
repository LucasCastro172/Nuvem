...
function step!(M::GradientDescent,f::Function,
    ...
    α = strong_backtracking(f, x, d, α=α)
...
function step!(M::Newton,f::Function,x::AbstractArray)
    ...
    α = strong_backtracking(f, x, d, α=α0) 
    if (α0 - α) < ϵ
        α = 1.0        
    end
...
rosenbrock2d(x; a=1.0, b=5.0) = b*(x[2] - x[1]^2)^2 + (a-x[1])^2
...
xo = [1.0, 1.0] # minimum point #
x0 = [-1.7, -1.7] # start point #
