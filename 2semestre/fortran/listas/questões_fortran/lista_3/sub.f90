...
Contains
!-----------------------------------
Subroutine cilindro(N,x,S,funcao)
implicit none
Real, intent(in):: x
Real :: nu, d
Real, intent(out):: S, funcao(n)
Real:: x_a(n)
Integer :: i,j
real:: eps
Integer,intent(in):: N ! numeros de pontos
...
!x(1)=0
  ...
  !   Read(22,*) a       
        x_a(j)= a
      Call trapezios(S,-R,R,eps,funcao(j))       
   End Do    
...
use integral_funcao
implicit none
Real, Allocatable,Dimension(:):: S, funcao
