...
implicit none 
integer:: N, g,nc
real(prc), allocatable:: x(:,:), y(:),ys(:)
real(prc), allocatable:: xtx(:,:), xty(:), p(:)
integer:: i

open(44, file='polinomio_mq.in', status='old',action='read')
Read(44,*)g
Read(44,*)N
 close(44)
nc=g+1 

allocate(x(n,nc),y(n),ys(n),xtx(nc,nc),xty(nc),p(nc))

x(:,1)=1.d0
open(55, file='Dados_para_ajustar.dat', status='old',action='read')
do i=1,n
   read(55,*) x(i,2), y(i)
end do
 close(55)
 ...
 ys=matmul(x,p)
open(22,file='solucao_MQ.dat',status='replace')
do i=1,N
