...
# O objetivo deste programa é de fazer análise de velocidades. O número de cmp é 10.
#  O primeiro cmp é 30 e o último é 174. O espaçamento entre cmp é 16. 
# Para determinar os demais cmp usamos uma progressão aritmética: a_{n}=a_{1} + (n-1)*r
# de modo que temos: a_{2}=46; a_{3}=62; a_{4}=78; a_{5}=94; a_{6}=110; a_{7}=126; 
# a_{8}=142; a_{9}=158 e a_{10}=174. 
#________________________________________________________________________________________
# set -x

cmps=camadas_planas_cdp.su
...
indata=$cmps
outdata=astur_velan.data #vpick
...
#------------------------------------------------------------
    ...
    suxwigb title="Seção CMP depois do NMO" xbox=1 ybox=10 \
